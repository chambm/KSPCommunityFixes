...
- New KSP bugfix : **WorldFrameDrift** Fix the planet's orientation in Unity's world axes drifting by the planet's rotation over the flight every time the clock is taken back over it (revert to VAB/SPH, mods reverting from their own time), which made stock's part-to-part thermal occlusion depend on what was flown before in the session. The frame is now re-derived from the clock on every scene change.
