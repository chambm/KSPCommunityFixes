...
- New KSP bugfix : **OcclusionVehicleFrame** Judge the convection, solar and body occlusion between parts in a frame fixed to the vessel instead of the world axes, so a part's shading by the parts in front of it no longer depends on heading, inclination, longitude or the session's history (a spaceplane's unshrouded aft engines peaked between 93 % and 131 % of their skin limit on the same entry depending on the orbit's orientation). Applies to stock's `OcclusionData.Update` and to the **FlightIntegratorPerf** reimplementation.
