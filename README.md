...
- [**WorldFrameDrift**](https://github.com/KSPModdingLibs/KSPCommunityFixes/pull/435) [KSP 1.8.0 - 1.12.5]<br/>Fix the planet's orientation in Unity's world axes (`CelestialBody.directRotAngle` / `Planetarium.InverseRotAngle`, neither of which is saved) drifting by the planet's rotation over the flight every time the clock is taken back over it - "Revert to VAB/SPH", or a mod reverting from its own time - cumulatively through the session. Everything judged in world axes, the axis-aligned rectangle test behind stock's part-to-part convection, solar and body occlusion in particular, then depends on what was flown before: the same vessel on the same launch gets different part shading and heating from one launch to the next. The frame is now re-derived from the clock on every scene change.
