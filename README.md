...
- [**OcclusionVehicleFrame**](https://github.com/KSPModdingLibs/KSPCommunityFixes/pull/436) [KSP 1.8.0 - 1.12.5]<br/>Fix part-to-part thermal occlusion (convection, solar and body) depending on heading, inclination and longitude. Stock bounds the parts in Unity's world axes, so how much a part is shaded by the parts in front of it changes with the vessel's orientation in the world; it is now computed in a frame fixed to the vessel. Also applies to the **FlightIntegratorPerf** reimplementation.
